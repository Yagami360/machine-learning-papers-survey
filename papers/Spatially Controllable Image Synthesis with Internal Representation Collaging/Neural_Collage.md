...
- Given a batch of images sam- pled from a single class, the conditional batch normalization [7, 5] normalizes the set of intermediate features produced from the batch by a pair of class-specific scale and bias pa- rameters. Let Fk,h,w represent the feature of l-th layer at the channel k, the height location h, and the width location w. Given a batch {Fi,k,h,w} of Fk,h,ws generated from a class c, the CBN at layer l normalizes Fi,k,h,w by:
